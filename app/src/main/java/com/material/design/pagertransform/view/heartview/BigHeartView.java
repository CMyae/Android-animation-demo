...
public class BigHeartView extends ImageView{

    public static int WIDTH;

    public BigHeartView(Context context,int width,int type) {
        super(context);
        WIDTH =  width;
        init(type);
    }

    public BigHeartView(Context context, AttributeSet attrs) {
        super(context, attrs);
        init(0);
    }

    public BigHeartView(Context context, AttributeSet attrs, int defStyleAttr) {
    ...
    void init(int type){
        setNonFavouriteImage(type);
        setScaleType(ScaleType.FIT_XY);
    }

    public void setFavouriteImage(int type){
        switch (type){
            case HEART_TYPE:
    ...
    public void setNonFavouriteImage(int type){
        switch (type){
            case HEART_TYPE:
                setImageResource(R.drawable.ic_favorite_not);
                break;
            case STAR_TYPE:
                setImageResource(R.drawable.ic_star_off);
                break;
            case THUMB_TYPE:
                setImageResource(R.drawable.ic_thumb_off);
                break;
            default:
                break;
        }
